...
def _serie_spy_ret(fecha_start: str):
    """Retorno simple de SPY. None si no hay datos válidos (nunca NaN)."""
...
def _serie_capital_desplegado(df_inv, df_tc, fecha_start, fecha_end):
    """Capital en juego por día (márgenes simultáneos). No suma trades reciclados."""
...
def calcular_rendimiento_periodo(user_id: int, periodo: str = "ytd"):
    """YTD / período: PnL + capital promedio real vs SPY."""
    hoy = datetime.now()
    ...
    lineas.append("")
    lineas.append("Nota: el % usa capital promedio simultáneo, no la suma de todos los trades (eso inflaba el número porque el mismo dinero se reutilizó). No incluye efectivo fuera del bot.")
    return "\n".join(lineas)
